...
export const getApplied_User = async (uid, pid) => {
  return await prisma.$queryRaw`SELECT a.post_id,a.user_id,a.description,a."created_at",u.name,u.image FROM "Applicants" a JOIN "Users" u ON a.user_id = u.id JOIN "Posts" p ON a.post_id = p.id WHERE p.user_id=${uid} AND a.post_id=${pid}`;
};
